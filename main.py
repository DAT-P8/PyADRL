...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        help=f"Run Ray Tune hyperparameter search. Examples available: {', '.join(examples)}",
    )
    parser.add_argument(
        "--restore",
        "-r",
    ...
    parser.add_argument(
        ...
        required=False,
        default=25,
        help="Number of hyperparameter configurations to try (only for --tune)",
    )
    parser.add_argument(
        "--max-concurrent",
        type=int,
        required=False,
        default=4,
        help="Max parallel trials during tuning (only for --tune). "
        "Lower this if you're memory-constrained.",
    )
...
def main():
    ...
    if args.tune:
        if args.tune == GRIDWORLD:
            print(
                f"Tuning gridworld example: {args.samples} samples, "
                f"max {args.max_concurrent} concurrent trials"
            )
            PyADRL.examples.gridworld.gridworld_tune(
                num_samples=args.samples,
                max_concurrent_trials=args.max_concurrent,
            )
        else:
            print(
                f"Unknown tune example: {args.tune}. Available examples: {', '.join(examples)}"
            )
