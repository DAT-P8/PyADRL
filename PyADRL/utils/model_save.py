...
def restore_checkpoint(algo, checkpoint_path: str, model_name: str | None = None):
    ...
    it will restore newest CP from that model. If it's a file, it will restore from that file."""

    if "/" in checkpoint_path: # if checkpoint_path contains a "/" we assume it's a direct path to a checkpoint file
        ...
        checkpoint_dir = setup_checkpoint_dir(model_name=model_name) # still set up a new checkpoint dir for saving future checkpoints
        ...
        existing_checkpoints = [d for d in os.listdir(checkpoint_dir) if re.match(r"cp_\d{5}", d)]
        if existing_checkpoints == []:
            raise ValueError(f"No checkpoints found in {checkpoint_dir} matching pattern cp_XXXXX")
        ...
        if model_name: # if model_name is provided, we save future checkpoints in that folder. 
            return setup_checkpoint_dir(model_name=model_name), 0
        else: 
            return checkpoint_dir, int(latest.split("_")[1]) # return the checkpoint dir and the iteration number to continue from


def restore_training(algo, checkpoint_path: str, pursuer_pool=None, evader_pool=None, model_name: str | None = None) -> tuple[str, int]:
    ...
    checkpoint_dir, iteration = restore_checkpoint(algo, checkpoint_path, model_name=model_name)
...
def setup_checkpoint_dir(model_name: str | None = None) -> str:
    ...
    if model_name:
        ...
        existing_models = [d for d in os.listdir(checkpoint_dir) if re.match(rf"{model_name}", d)]
        if existing_models:
            raise ValueError(f"Model name {model_name} already exists. Please choose a different name or delete the existing model.")
        return os.path.join(checkpoint_dir, model_name)
    else: 
        # We match on "model_xxx" where xxx is a number
        existing_checkpoints = [d for d in os.listdir(checkpoint_dir) if re.match(r"model_\d{3}", d)]
        if existing_checkpoints:
            latest = sorted(existing_checkpoints)[-1]
