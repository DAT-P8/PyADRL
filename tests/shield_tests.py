...
def assert_safe_actions(actions: list[DroneAction], state: State, map: MapSpec):
    ...
    for drone in state.drone_states:
        ...
        new_pos = (
            current_pos + utility.action_to_vector(action.action) * action.velocity
        )
    ...
    for id in new_positions:
        ...
        if (
            new_position[0] < 0
            or xb <= new_position[0]
            or new_position[1] < 0
            or yb <= new_position[1]
        ):
            raise Exception(
                f"This position was out of bounds: ({new_position[0]}, {new_position[1]})"
            )
...
def get_drone(
    ...
    new_pos = utility.action_to_vector(action) * velocity + np.array(
